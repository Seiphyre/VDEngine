...
Texture::Texture(const std::string text_name)
{
    ...
    // Texture filtering can be set for magnifying (GL_TEXTURE_MAG_FILTER) and minifying (GL_TEXTURE_MIN_FILTER) operations (when scaling up or downwards)
    // so you could for example use nearest neighbor filtering when textures are scaled downwards and linear filtering for upscaled textures.
...
void Texture::Create(const std::string text_name)
{
    ...
    if (textureImage->GetData() != NULL)
    {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0 /* Mipmap level */, format, textureImage->GetWidth(), textureImage->GetHeight(), 0, format, GL_UNSIGNED_BYTE, textureImage->GetData());
