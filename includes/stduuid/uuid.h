...
namespace detail
{
template <typename TChar> constexpr inline unsigned char hex2char(TChar const ch)
{
...
template <typename TChar> constexpr inline bool is_hex(TChar const ch)
{
...
template <typename TChar> constexpr inline unsigned char hexpair2char(TChar const a, TChar const b)
{
...
// version number.
// clock_seq_hi_and_reserved	unsigned small	   8	      The high field of the clock sequence multiplexed with the
// variant. clock_seq_low	            unsigned small	   9	      The low field of the clock sequence. node
...
class uuid
{
  public:
    ...
    template <class CharT = char> static bool is_valid_uuid(CharT const * str) noexcept
    {
        CharT  digit      = 0;
    ...
    template <class CharT = char> static std::optional<uuid> from_string(CharT const * str) noexcept
    {
        CharT  digit      = 0;
        bool   firstDigit = true;
        int    hasBraces  = 0;
...
{
    return s << std::hex << std::setfill(static_cast<Elem>('0')) << std::setw(2) << (int)id.data[0] << std::setw(2)
             << (int)id.data[1] << std::setw(2) << (int)id.data[2] << std::setw(2) << (int)id.data[3] << '-'
             << std::setw(2) << (int)id.data[4] << std::setw(2) << (int)id.data[5] << '-' << std::setw(2)
             << (int)id.data[6] << std::setw(2) << (int)id.data[7] << '-' << std::setw(2) << (int)id.data[8]
             << std::setw(2) << (int)id.data[9] << '-' << std::setw(2) << (int)id.data[10] << std::setw(2)
             << (int)id.data[11] << std::setw(2) << (int)id.data[12] << std::setw(2) << (int)id.data[13] << std::setw(2)
             << (int)id.data[14] << std::setw(2) << (int)id.data[15];
}
  ...
  public:
    ...
    uuid operator()()
    {
        ...
        std::array<uint8_t, 16> bytes = {
            {(unsigned char)((newId.Data1 >> 24) & 0xFF), (unsigned char)((newId.Data1 >> 16) & 0xFF),
             (unsigned char)((newId.Data1 >> 8) & 0xFF), (unsigned char)((newId.Data1) & 0xFF),
...
class uuid_name_generator
{
  public:
    ...
    template <class CharT = char> uuid operator()(CharT const * name)
    {
        size_t size = 0;
        if constexpr (std::is_same_v<CharT, char>)
            size = strlen(name);
